...
public class TransactionController {
		...
		@GetMapping("/amount-by-subcategory")
		@Operation(summary = "Retornar os valores de entrada e saida do mês do usuario", description = "Essa função é responsável retornar os valores de entrada ou saida do mês do usuario dependendo do tipo da transação")
		public ResponseEntity<List<TransactionSubcategoryAmountDTO>> getAmountCurrentTransactionsBySubCategory(
						@RequestParam UUID userId,
						@RequestParam int month,
						@RequestParam int year
					 ) {
				var result = this.transactionService.getAmountCurrentTransactionsBySubCategory(userId, month, year);
				return new ResponseEntity<>(result, HttpStatus.OK);
		}
		...
		public ResponseEntity<TransactionResponseDTO> deleteTransaction(@PathVariable UUID id,
						@RequestParam UUID userId) {
				var result = this.transactionService.deleteTransaction(id, userId);
				return new ResponseEntity<>(result, HttpStatus.OK);
		}
